<#!
.SYNOPSIS
  Builds Subnautica configuration and generates a per-project compile status report (Markdown + JSON).

.DESCRIPTION
  Runs the existing build-all.ps1 with -Subnautica, tees output to docs/build-logs, parses per-project results
  from the standard "succeeded/failed" lines, and writes a summary matrix to docs/build-status/Subnautica.
...
$statusDir = Join-Path $repo 'docs/build-status/Subnautica'
New-Item -ItemType Directory -Force -Path $logsDir, $statusDir | Out-Null

$logFile = Join-Path $logsDir ("Subnautica-" + $ts + '.log')

$buildArgs = @('-Subnautica','-Verbose')
...
# Enumerate projects from Subnautica.slnf
$slnfPath = Join-Path $repo 'Subnautica.slnf'
if (-not (Test-Path $slnfPath)) { throw "Subnautica.slnf not found at $slnfPath" }
...
foreach ($rel in $projectRelPaths) {
  ...
  # OutputPath is bin/$(Configuration)/$(ProjectName)/; assume $(ProjectName) == project file name
  $dllPath = Join-Path $repo ("bin/Subnautica/$projName/$asmName.dll")
...
# Build JSON and Markdown summaries
